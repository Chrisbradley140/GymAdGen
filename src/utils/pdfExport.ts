...
export const generateCampaignPDF = async (campaign: CampaignWithContent) => {
  ...
  // Helper function to add logo image to footer
  const addLogoToFooter = async () => {
    try {
      ...
      return new Promise((resolve) => {
        logoImg.onload = () => {
          ...
          // Position logo in footer area: right side, vertically centered in footer
          pdf.addImage(logoData, 'PNG', pageWidth - margin - 12, pageHeight - 12, 10, 10);
          resolve(true);
        };
  ...
  // Add branded footer with logo to all pages
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    // Add logo to footer first (so text doesn't overlap)
    await addLogoToFooter();
    
    // Footer text
    pdf.setFontSize(8);
    pdf.setTextColor(100, 100, 100);
    pdf.text(`Page ${i} of ${totalPages}`, margin, pageHeight - 7);
    pdf.text('Generated by FITNESSADS.AI', margin + 80, pageHeight - 7);
  }
